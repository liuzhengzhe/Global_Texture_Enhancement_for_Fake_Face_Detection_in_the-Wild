...
model=torch.load('graminit.pth')
#model.load_state_dict(resnetinit.state_dict(),strict=False)
#model=torch.load('resinit.pth')
print (model.modules)
...
def adjust_learning_rate(optimizer,epoch):
  if epoch<5:
    ...
    lr=0.00011-0.00001*epoch
  elif epoch>=10:
    lr=0.00002-0.000001*epoch
...
for epoch in range(epochs):
    ...
    torch.save(model, 'aerialmodelepoch'+str(epoch)+'.pth')
        #if steps%100==0:
        #  torch.save(model, 'aerialmodel'+str(len(glob.glob('*.pth')))+'.pth')
